...
const ClubAthleteMedalRecap: React.FC<{ events: SwimEvent[], swimmers: Swimmer[], info: CompetitionInfo, brokenRecords: BrokenRecord[], selectedClub: string }> = ({ events, swimmers, info, brokenRecords, selectedClub }) => {
    ...
    return (
// FIX: Added the missing PrintView component export to resolve the import error in App.tsx.
// --- MAIN COMPONENT ---
export const PrintView: React.FC<PrintViewProps> = ({ events, swimmers, competitionInfo, isLoading }) => {
    type ReportType = 'schedule' | 'program' | 'results' | 'club_medals' | 'individual_medals' | 'broken_records' | 'category_winners' | 'club_recap';
    ...
    const brokenRecords = useMemo(() => {
        ...
        return [...new Map(brokenRecordsList.map(item => [item.record.id, item])).values()];
    }, [events, swimmers, records]);
    
    const allClubs = useMemo(() => [...new Set(swimmers.map(s => s.club))].sort(), [swimmers]);

    const generateReport = (type: ReportType) => {
        setSelectedReport(type);
        switch (type) {
            case 'schedule':
                setPrintTitle('Jadwal Acara');
                setPreviewContent(<ScheduleOfEvents events={events} />);
                break;
            case 'program':
                setPrintTitle('Buku Program');
                setPreviewContent(<ProgramBook events={events} swimmers={swimmers} info={competitionInfo!} records={records} />);
                break;
            case 'results':
                setPrintTitle('Hasil Lomba Lengkap');
                setPreviewContent(<EventResults events={events} swimmers={swimmers} info={competitionInfo!} records={records} brokenRecords={brokenRecords} />);
                break;
            case 'club_medals':
                setPrintTitle('Klasemen Medali Klub');
                setPreviewContent(<ClubMedalStandings events={events} swimmers={swimmers} info={competitionInfo!} />);
                break;
            case 'individual_medals':
                setPrintTitle('Klasemen Medali Perorangan');
                setPreviewContent(<IndividualStandings events={events} swimmers={swimmers} info={competitionInfo!} records={records} />);
                break;
            case 'broken_records':
                setPrintTitle('Rekapitulasi Pemecahan Rekor');
                setPreviewContent(<BrokenRecordsReport brokenRecords={brokenRecords} info={competitionInfo!} />);
                break;
            case 'category_winners':
                setPrintTitle('Rekap Juara per Kategori');
                setPreviewContent(<RekapJuaraPerKategori events={events} swimmers={swimmers} info={competitionInfo!} />);
                break;
            case 'club_recap':
                setPrintTitle(`Rekap Medali Klub: ${selectedClub === 'all' ? 'Semua Klub' : selectedClub}`);
                setPreviewContent(<ClubAthleteMedalRecap events={events} swimmers={swimmers} info={competitionInfo!} brokenRecords={brokenRecords} selectedClub={selectedClub} />);
                break;
        }
    };
    
    useEffect(() => {
        if (selectedReport === 'club_recap') {
            generateReport('club_recap');
        }
    }, [selectedClub]);


    if (isLoading || !competitionInfo) {
    ...
    return (
        <div>
            <Card className="no-print mb-6">
                <h1 className="text-3xl font-bold mb-4">Unduh & Cetak Laporan</h1>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    <Button onClick={() => generateReport('schedule')}>Jadwal Acara</Button>
                    <Button onClick={() => generateReport('program')}>Buku Program</Button>
                    <Button onClick={() => generateReport('results')}>Hasil Lomba</Button>
                    <Button onClick={() => generateReport('club_medals')}>Klasemen Klub</Button>
                    <Button onClick={() => generateReport('individual_medals')}>Klasemen Perorangan</Button>
                    <Button onClick={() => generateReport('broken_records')}>Pemecahan Rekor</Button>
                    <Button onClick={() => generateReport('category_winners')}>Juara per Kategori</Button>
                     <div className="col-span-2 md:col-span-1">
                        <select
                            value={selectedClub}
                            onChange={e => setSelectedClub(e.target.value)}
                            className="w-full h-full bg-background border border-border rounded-md px-3 py-2 text-text-primary focus:outline-none focus:ring-2 focus:ring-primary"
                        >
                            <option value="all">Rekap Medali Semua Klub</option>
                            {allClubs.map(club => <option key={club} value={club}>{club}</option>)}
                        </select>
                     </div>
                     <Button onClick={() => generateReport('club_recap')} className="col-span-2 md:col-span-3 lg:col-span-1">
                        Tampilkan Rekap Medali Klub
                    </Button>
                </div>
                 <div className="mt-6 pt-4 border-t border-border flex justify-end">
                    <Button onClick={() => window.print()} disabled={!selectedReport}>Cetak Laporan</Button>
                </div>
            </Card>
